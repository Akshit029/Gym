...
const Login = ({ toggleForm }) => {
  ...
  const handleLogin = (e) => {
    ...
    // Simulating API call (replace with actual login logic)
    // Assuming login is successful if email and password are filled
  ...
  return (
    <div className="transition-opacity duration-500 ease-in-out bg-transparent">
      ...
      <form onSubmit={handleLogin} className="rounded-full px-8 pt-6 pb-8 mb-4 bg-transparent border-none outline-none">
        {/* Login form fields */}
        <div className="mb-4">
      ...
      <p className="text-center text-black mt-4 font-bold">
        Don't have an account? <button onClick={toggleForm} className="text-white hover:text-blue-700">Sign up</button>
      </p>
    </div>
  );
};
